...
class Client:
    ...
    def __init__(
        self, client_id: str, client_secret: str, *, scopes=[], app: App = None
    ):
        ...
        self.serializer = URLSafeSerializer(
            os.getenv("SECRET_KEY"), salt=os.getenv("SECRET_SALT").encode()
        )
...
async def toc(request: Request):
    return templates.TemplateResponse("priv_pol.html", {"request": request})
...
async def load_more_playlist_tracks(
    request: Request, playlist_id: str, page: int, user: User = get_user
):
    if not user:
        return RedirectResponse("/login")
    offset = page * 20
...
async def load_more_toptracks(
    request: Request, page: int, type: str = "short_term", user: User = get_user
):
    if not user:
        return RedirectResponse("/login")
    offset = page * 20
...
async def top_artists(
    request: Request, type: str = "short_term", user: User = get_user
):
    if not user:
        return RedirectResponse("/login")
    return templates.TemplateResponse(
...
async def load_more_topartists(
    request: Request, page: int, type: str = "short_term", user: User = get_user
):
    if not user:
        return RedirectResponse("/login")
...
async def callback(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "loggedin.html", {"request": request, "user": user}
        )
    except Exception as e:
        logger.error(f"Error during callback: {e}\n{traceback.format_exc()}")
        if "user may not be registered" in str(e):
            return {
                "error": "User is not registered as a tester. Please contact the administrator."
            }
